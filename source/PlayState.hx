...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			GameOverSubstate.characterName = 'bfgb2';

			addCharacterToList('bfgb2', 0);
					...
					bgfeo = new BGSprite('mario/EXE2/Mario_Phase2_Background_Assets_Static', 200, 300, 0.1, 0.1, ['Estatica papu instancia '], true);
					...
					gfwasTaken = new BGSprite('characters/gfland', 400 + 190, 100 + 310 + 450, 0.9, 0.9, ['gfland idle'], true); // WTF GF ES UN PROP
					gfwasTaken.setGraphicSize(Std.int(gfwasTaken.width * 5));
					gfwasTaken.antialiasing = false;
					gfwasTaken.alpha = 0;
					add(gfwasTaken);
					gfwasTaken.animation.addByPrefix('gfland idle', 'gfland idle', 12, true);
					gfwasTaken.animation.play('gfland idle');
			...
			GameOverSubstate.characterName = 'bfvhs2';

			addCharacterToList('bfvhs2', 0);
			...
			GameOverSubstate.characterName = 'bfnew2';

			addCharacterToList('bfnew2', 0);
		...
		if(curStage == 'exeport'){
			...
			add(shadowbg);
				...
				var vcr = new VCRMario85();

				camGame.setFilters([new ShaderFilter(vcr)]);
				camEst.setFilters([new ShaderFilter(vcr)]);
		}
	...
	/*	if(dad.curCharacter.startsWith('mariano')) {
		...
		add(blackBarThingie);
			...
			mxLaugh = new BGSprite('mario/MX/MX_Dialogue_Asseta', -1150, 200, ['Innocence'], false);
			mxLaugh.animation.addByPrefix('freddyfazbear', "Innocence", 32, false);
			mxLaugh.antialiasing = ClientPrefs.globalAntialiasing;
			mxLaugh.alpha = 0.000001;
	...
	public function startCountdown():Void
	{
		...
		if (curStage == 'betamansion')
		{
			...
			gfGroup.alpha = 0.8;
			extraTween.push(FlxTween.tween(gfGroup, {y: gfGroup.y + 40}, 2, {startDelay: 0.2, ease: FlxEase.quadInOut, type: PINGPONG}));
			extraTween.push(FlxTween.tween(gfGroup, {x: gfGroup.x - 20}, 4, {startDelay: 0.2, ease: FlxEase.quadInOut, type: PINGPONG}));
		}
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			if (curStage == 'exeport'){
				...
				var blackthing = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, FlxColor.BLACK);
					...
					eventTweens.push(FlxTween.tween(blackthing, {alpha: 0}, 1, {ease: FlxEase.quadInOut}));
			}
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.ENTER && startedCountdown && canPause)
		{
			...
			if(healthDrain > 0)
			{
				...
				eventTimers.push(new FlxTimer().start(0.5, function(gef:FlxTimer)
				{
						whenCuentas += 20;
						health -= healthDrain;
			...
			if (curStage == 'hatebg' && !boyfriend.animation.curAnim.name.endsWith('miss'))
			{
		...
		if (controls.DODGE && !inCutscene && !endingSong && !isDodging && !canDodge && !cpuControlled && !startingSong && curStage == 'exeport'){ //PONELE MÁS REQUISITOS LA CTM
			bfJump();
		}
	...
	function doDeathCheck() {
		if (health <= 0 && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				if(GameOverSubstate.characterName == 'bfgb2')
				{
				boyfriendGroup.x = BF_X;
				boyfriendGroup.y = BF_Y;
				boyfriend.x = BF_X + 0;
				boyfriend.y = BF_Y + 410;
				}

				if(GameOverSubstate.characterName == 'bfnew2' || GameOverSubstate.characterName == 'bfvhs2')
				{
				boyfriendGroup.x = BF_X;
				boyfriendGroup.y = BF_Y;
				boyfriend.x = BF_X + 0;
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Play Animation':
				...
				if(value1 == 'Hey' && char == dad && curStage == 'execlassic' && dad.curCharacter == 'mariano')
				{
				triggerEventNote('Change Character', '1', 'mariohorrorpissed');
				char = dad;
				}
				if(value1 == 'danceRight' && char == gf && curStage == 'racing')
				char.dance();
				else
				{
		...
		switch(triggerGL){
			...
			for (land in landbg)
			{
			eventTweens.push(FlxTween.tween(land, {alpha: 0}, tiempoN, {ease: FlxEase.quadInOut}));
			}
			eventTweens.push(FlxTween.tween(bricksland, {alpha: 0}, tiempoN, {ease: FlxEase.quadInOut}));
			eventTweens.push(FlxTween.tween(brickslandEXE, {alpha: 1}, tiempoN, {ease: FlxEase.quadInOut}));
			...
			case 1:
			eventTweens.push(FlxTween.tween(gfwasTaken, {y: gfwasTaken.y - 450}, 1, {ease: FlxEase.quadOut}));
			case 2:
		...
		case 'MX salto':
			...
			eventTweens.push(FlxTween.tween(dad, {y: enemyY - numberjump}, 0.24, {startDelay: 0.24, ease: FlxEase.quadOut, onComplete: function(twn:FlxTween)
				{
					if (cpuControlled) bfJump();
					eventTweens.push(FlxTween.tween(dad, {y: enemyY}, 0.24, {ease: FlxEase.quadIn, onComplete: function(twn:FlxTween)
					{
							if(ClientPrefs.flashing)
							{
		...
		case 'Triggers Innocent':
			...
			switch(triggerMX){
				case 0:
					eventTweens.push(FlxTween.tween(blackBarThingie, {alpha: 1}, 0.5, {ease: FlxEase.quadInOut, onComplete: function(twn:FlxTween)
					{
									mxLaugh.y = 200;
									mxLaugh.scale.set(0.8, 0.8);
									eventTweens.push(FlxTween.tween(mxLaugh.scale, {x: 1, y: 1}, 1.3, {startDelay: 1, ease: FlxEase.cubeOut}));
									eventTweens.push(FlxTween.tween(mxLaugh, {y: 80}, 1.3, {startDelay: 1, ease: FlxEase.cubeOut}));
									eventTweens.push(FlxTween.tween(mxLaugh, {alpha: 1}, 0.3, {startDelay: 1, ease: FlxEase.quadInOut}));
									eventTimers.push(new FlxTimer().start(0.375, function(tmr:FlxTimer)
									{
										mxLaugh.animation.play('freddyfazbear');
									}));
					}}));

				case 1:
					eventTweens.push(FlxTween.tween(mxLaugh, {alpha: 0}, 0.5, {startDelay: 0.2, ease: FlxEase.quadInOut}));
				...
				case 2:
					...
					triggerEventNote('Change Character', '0', 'bfsad');
			}
		case 'Triggers IHY':
		...
		case 'Triggers Racing':
			...
			switch(triggerMR){
				case 0:
						dad.x = DAD_X + dad.positionArray[0];
						dad.alpha = 1;
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			case 'execlassic':
				if (curBeat == 120)
				triggerEventNote('Change Character', '1', 'mariano');
			case 'landstage':
